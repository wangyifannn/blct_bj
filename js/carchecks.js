...
$("#carCheck form .vSn").on('change', function() {
    let vSn = $(this).val()
    $("#newcarTypeIn .cartypein_apply .vSn").val(vSn).attr("readonly", true);
    ToolRecordApply("#newcarTypeIn .rd_apply", vSn, "");
    bomApply("#newcarTypeIn .bom_apply", vSn, "");
})

$(".carCheck").unbind('click').bind('click', function() {
    $("#carCheck input").val("");
    myformReset("#carCheck"); //表单重置
...
$("#newcarTypeIn .sub_cartypein").unbind('click').bind('click', function() {
    ...
    var numreg = /^\d+(\.\d+)?$/;
    if (numreg.test($("#newcarTypeIn .vehicleQuality").val()) == false) {
        toastr.warning('吨位格式不正确，只能输入数字或带小数点的数字', '提示', messageOpts);
        return;
    }
    // $.ajax({
    ...
    setTimeout(function() {
        ...
        //     $.ajax({ //上传bom文件
        //         url: allurl + "/car-management/car/bomUpload.action?vSn=" + $("#newcarTypeIn .cartypein_apply .vSn").val(),
        //         type: "post",
        //         async: false,
        //         data: bomformData,
        //         // headers: {
        ...
        //         },
        //         // data: new FormData($('#newbom_file')[0]),
        //         processData: false,
        //         contentType: false,
        //         success: function(res) {
        //             if (res.ret == true) {
...
$("#carupCheck .form_subject .pull-right").unbind('click').bind('click', function() {
    ...
    $("#carupCheck .project_name").attr("readOnly", true);
...
$("#carTypeIn .sub_cartypein").unbind('click').bind('click', function() {
    ...
    var numreg = /^\d+(\.\d+)?$/;
    if (numreg.test($("#carTypeIn .vehicleQuality").val()) == false) {
        toastr.warning('吨位格式不正确，只能输入数字或带小数点的数字', '提示', messageOpts);
        return;
    }
